<%@ page isErrorPage="true" %>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"
    integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <title>Error</title>
</head>
<body >
    <!-- Nav Bar -->
<nav class="navbar navbar-dark bg-light">
    <div class="container">
        <span class="navbar-brand mb-0 h1"><img src="img/sa.png" width="54" height="55"></span>
        <a href="index.jsp" class="btn btn-outline-success" role="button">Login</a>
    </div>
</nav>
    <div class="p-5 mb-4 bg-light rounded-3" style="background-color:#66FFCC">
      <div class="container-fluid py-5">
        <h1 class="display-5 fw-bold">404 Error! Page Not Found.</h1>
        <p class="col-md-8 fs-4"> <strong> Sorry!! </Strong></p>
        <a href="javascript:history.back()" class="btn btn-outline-success" role="button">Back</a>
      </div>
</body>
</html>